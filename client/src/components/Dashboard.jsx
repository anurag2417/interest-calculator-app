...
const Dashboard = () => {
    ...
    const downloadPDF = () => {
        ...
        // Table Data
        const tableColumn = ["Date", "Name", "Type", "Amount", "Interest", "Total Due"];
        const tableRows = [];
    ...
    return (
        ...
        >
            ...
            <div className="dashboard-header" style={{
                ...
                <div>
                    ...
                    <p style={{color:'#94a3b8', margin:'5px 0 0 0'}}>
                        Welcome back, <span style={{color: '#fff', fontWeight:'bold'}}>{user?.name}</span>
                    </p>
                </div>
            ...
            <motion.div className="table-container" variants={itemVariants}>
                {loading ? (
                    <p style={{textAlign:'center', padding:'20px', color:'#94a3b8'}}>Loading transactions...</p>
                ) : filteredTransactions.length === 0 ? (
                    <p style={{textAlign:'center', padding:'40px', color:'#94a3b8'}}>No transactions found.</p>
                ) : (
                                ...
                                return (
                                    <tr key={t._id} style={{opacity: isSettled ? 0.5 : 1}}>
                                        <td>
                                            {new Date(t.date).toLocaleDateString()}
                                            {t.dueDate && (
                                                <div style={{fontSize:'0.75em', color:'#f59e0b', marginTop:'4px'}}>
                                        ...
                                        <td>
                                            {t.personName}
                                            {isSettled && <span className="badge badge-paid" style={{marginLeft:'8px'}}>PAID</span>}
                                        </td>
                                        <td>
                                            <span style={{
                                                color: t.type === 'Given' ? '#10b981' : '#ef4444',
                                                fontWeight: '600',
                                                background: t.type === 'Given' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                                                padding: '4px 8px',
                                                borderRadius: '6px'
                                            }}>
                                                {t.type}
                                        ...
                                        <td style={{fontWeight:'bold', color: '#fff'}}>₹{result.total}</td>
                                        <td>
                                            <button 
